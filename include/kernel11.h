...
void scale_c_k11(double *C,int M, int N, int LDC, double scalar){
    int i,j;
    for (i=0;i<M;i++){
        for (j=0;j<N;j++){
            C(i,j)*=scalar;
...
void packing_b_24x8_version1_k11(double *src,double *dst,int leading_dim,int dim_first,int dim_second){
...
void packing_a_24x8_k11(double *src, double *dst, int leading_dim, int dim_first, int dim_second){
    ...
    int LDC)\
{
    ...
    for (k_count=0;k_count<K;k_count+=k_inc){
        ...
        for (m_count=0;m_count<M_MAIN;m_count+=M_BLOCKING){
            packing_a_24x8_k11(A+m_count+k_count*LDA,a_buffer,LDA,M_BLOCKING,k_inc);
            for (n_count=0;n_count<N_MAIN;n_count+=N_BLOCKING){
                packing_b_24x8_version1_k11(B+k_count+n_count*LDB,b_buffer,LDB,k_inc,N_BLOCKING);
            ...
            if (N8-N_MAIN>0){
                packing_b_24x8_version1_k11(B+k_count+N_MAIN*LDB,b_buffer,LDB,k_inc,N8-N_MAIN);
