...
void scale_c_k13(double *C,int M, int N, int LDC, double scalar){
    int i,j;
    for (i=0;i<M;i++){
        for (j=0;j<N;j++){
            C(i,j)*=scalar;
...
void packing_b_24x8_edge_version2_k13(double *src,double *dst,int leading_dim,int dim_first,int dim_second){
...
void packing_a_24x8_edge_k13(double *src, double *dst, int leading_dim, int dim_first, int dim_second){
...
void kernel_n_8_v2_k13(double *a_buffer,double *b_buffer,double *c_ptr,int m,int K,int LDC,double alpha){
...
void kernel_n_4_v2_k13(double *a_buffer,double *b_buffer,double *c_ptr,int m,int K,int LDC,double alpha){
    int m_count,m_count_sub;
    int i,j,k;
    double *C=c_ptr;
    double sc0,sc1,sc2,sc3,sc4,sc5,sc6,sc7,sa,sb0,sb1,sb2,sb3,sb4,sb5,sb6,sb7;
...
void macro_kernel_v2_k13(double *a_buffer,double *b_buffer,int m,int n,int k,double *C, int LDC,int k_inc,double alpha){
    ...
    for (n_count_sub=n,n_count=0;n_count_sub>7;n_count_sub-=8,n_count+=8){
        ...
        kernel_n_8_v2_k13(a_buffer,b_buffer+n_count*k_inc,C+n_count*LDC,m,k_inc,LDC,alpha);
    }
    for (;n_count_sub>3;n_count_sub-=4,n_count+=4){
        //call the m layer with n=4
        kernel_n_4_v2_k13(a_buffer,b_buffer+n_count*k_inc,C+n_count*LDC,m,k_inc,LDC,alpha);
    }
...
{
    int i,j,k;
    if (beta != 1.0) scale_c_k13(C,M,N,LDC,beta);
    ...
    for (k_count=0;k_count<K;k_count+=k_inc){
        ...
        for (n_count=0;n_count<N;n_count+=n_inc){
            ...
            packing_b_24x8_edge_version2_k13(B+k_count+n_count*LDB,b_buffer,LDB,k_inc,n_inc);
            //print_matrix(b_buffer,k_inc,n_inc);
            for (m_count=0;m_count<M;m_count+=m_inc){
                m_inc=(M-m_count>OUT_M_BLOCKING)?OUT_M_BLOCKING:M-m_count;
                packing_a_24x8_edge_k13(A+m_count+k_count*LDA,a_buffer,LDA,m_inc,k_inc);
                ...
                for (second_m_count=m_count;second_m_count<m_count+m_inc;second_m_count+=second_m_inc){
                    ...
                    for (second_n_count=n_count;second_n_count<n_count+n_inc;second_n_count+=second_n_inc){
                        ...
                        macro_kernel_v2_k13(a_buffer+(second_m_count-m_count)*k_inc,b_buffer+(second_n_count-n_count)*k_inc,second_m_inc,second_n_inc,k_inc,&C(second_m_count,second_n_count),LDC,k_inc,alpha);
