...
void packing_a_24x8_edge_k13(double alpha,double *src, double *dst, int leading_dim, int dim_first, int dim_second){
...
void macro_kernel_k13(double *a_buffer,double *b_buffer,int m,int n,int k,double *C, int LDC){
    ...
    for (n_count=0;n_count<N;n_count+=n_inc){
        ...
        for (k_count=0;k_count<K;k_count+=k_inc){
            ...
            for (m_count=0;m_count<M;m_count+=m_inc){
                ...
                packing_a_24x8_edge_k13(alpha, A+m_count+k_count*LDA,a_buffer,LDA,m_inc,k_inc);
                //macro kernel: to compute C += A_tilt * B_tilt
                macro_kernel_k13(a_buffer, b_buffer, m_inc, n_inc, k_inc, &C(m_count, n_count), LDC);
            }
        }
    }
